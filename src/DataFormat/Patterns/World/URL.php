 ...
 * @license     MIT
 */
final class URL extends aStringFormat
{
     ...
     * @var ?string
     */
    const RegExp = "/^(((http)|(https)|(ftp)|(ftps))(:\/\/))?([\w\-]{1,256})([.]{1,1})([\w#!:.?+=&%@\-\/]{1,1783})$/";
    ...
    public static function format($v, ?array $aux = null): ?string
    {
        if (\is_string($v) === true) {
            if ($v === "" || \mb_strlen($v) < 6 || (\mb_strpos($v, "http://") === false &&
                \mb_strpos($v, "https://") === false &&
                \mb_strpos($v, "ftp://") === false &&
                \mb_strpos($v, "ftps://") === false)) {
                $v = "http://" . $v;
            }
